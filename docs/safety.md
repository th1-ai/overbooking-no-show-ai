...
## Overbooking & No-Show AI specifics

The two brakes above (`mode: shadow`, `--dry-run`) are the whole family's
safety model. This agent adds guardrails of its own, on top:

**Never walks a guest.** Structurally, not by policy: there is no code path
in this repo that contacts a guest, and no adapter call that mutates a
reservation. The agent ranks disruption and prices the alternative; a human
at the desk decides, and dials the phone. This is the agent's defining
constraint (see `docs/how-it-works.md` §6 of the spec, carried through
unchanged), and it holds in `mode: shadow` and `mode: live` alike - going
live never adds a guest-contact or reservation-write capability that did not
exist before.

**Oversell capped, always.** `oversell.max_buffer` (default 3, matching the
spec) is a hard ceiling regardless of how many no-shows are predicted. The
`rules.oversell_buffer` toggle, when off, forces the recommendation to 0
"whatever the prediction says" - there is no config combination that
overrides the cap or the rule.

**Floor, not round.** Expected no-shows are floored before becoming a
buffer - a safe-side rounding choice baked into the arithmetic, not a
setting.

**A recorded no-show is never a walk candidate.** `build_candidates()`
excludes anyone already resolved as `no_show` or `cancelled` structurally -
a guest who will not be there tonight cannot be walked.

**Loyalty and long stays are protected in the walk score**, not by an
override list - `+10` for loyalty is the single largest term in
`walk_score()`, ahead of channel or lead time. See `docs/how-it-works.md`
"The walk plan".

**The agent never marks a reservation `arrived` or `no_show`.** That is your
property's own night-audit process. `tools/run.py --once --reconcile` only
reads. See `docs/how-it-works.md` "Design decisions" #7.

**No card data, no payments.** `walk.taxi_cost` and `walk.goodwill_credit`
are priced and logged; nothing in this repo posts a charge, a refund, or a
credit to any ledger. `Payments`/`Accounting` are stub adapters this agent
never calls.

**No LLM anywhere.** There is no prompt, no model call, and therefore no
prompt-injection surface, no hallucination risk, and no data leaving this
machine even with `llm.provider: anthropic` configured for the rest of the
family - this agent simply does not use it. See "Design decisions" #0.
