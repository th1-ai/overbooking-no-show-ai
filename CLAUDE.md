...
You are working inside **Overbooking & No-Show AI** ("The Juggler") — Predicts no-shows and late cancellations per night from booking history and lead-time patterns, recommends a safe controlled-overbooking level, and preps the walk plan (which booking, which partner hotel) if the night runs over..
...
**No LLM step, anywhere — read this before "The interactive provider"
below.** Unlike every other repo in this family, this agent never calls a
model — not `mock`, not `interactive`, not `claude-code`, not `anthropic`.
Every number it produces is a formula over PMS data (see
`docs/how-it-works.md` "Design decisions" #0). `config/hotel.yaml` still
carries an `llm:` block and `make doctor` still prints an `llm provider`
line — that is boilerplate shared with the rest of the family, not a sign
this agent uses one. **The whole "interactive provider" section right below
does not apply to this repo — skip it.** There is no `prompts/` folder and
no `data/pending/*.prompt.md` to answer, ever. Full detail: "Agent
specifics" at the end of this file.
...
**No LLM step, anywhere.** Unlike every other repo in this family, this
agent never calls a model - not `mock`, not `interactive`, not
`claude-code`, not `anthropic`. Every number it produces is a formula over
PMS data. Do not look for a `prompts/` folder; there is not one. See
`docs/how-it-works.md` "Design decisions" #0.

**Main workflow:** `workflows/10-scan.md` (score tonight's arrivals,
recommend a buffer, prep the walk plan) and `workflows/11-reconcile.md` (the
next morning, read back what actually happened). Both run through
`tools/run.py`: `--once` scans, `--once --reconcile` reads back.

**Sub-agents:** none. The Juggler is a standalone, top-level agent - it
shares the demo platform's `/pricing` page with Revenue Management AI ("The
Quant") but has its own repo, its own engine, and no dependency on the
Quant's repricing logic.

**What needs a human:**
- Every recommended buffer above 0 - published only after
  `python3 tools/review.py approve` or `edit`, never automatically, in
  shadow or in live mode.
- Every walk itself. This agent ranks disruption and prices the
  alternative; it never contacts a guest and never decides who actually
  moves. That is always the duty manager, using the ladder as a starting
  point.
- `needs_human` items: `hotel.rooms` not configured, or no rate on file for
  `reference_room_type`. `make doctor` names the exact gap.

**Read `docs/how-it-works.md` before changing the risk formula or the walk
score.** Every weight is a named config knob in `config/agent.yaml`; the doc
explains what each one does and why the defaults are what they are.
